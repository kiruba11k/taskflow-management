...
export default function AddMemberForm({ open, onOpenChange, onMemberAdded }: AddMemberFormProps) {
  const initialSkills: Skills = Object.keys(skillLabels).reduce((acc, key) => {
    acc[key] = 5;
    return acc;
  }, {} as Skills);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onMemberAdded();
      onOpenChange(false);
      setFormData({ ...formData, name: "", email: "", role: "", skills: initialSkills, availability: 100, current_workload: 0 });
    } catch (error) {
